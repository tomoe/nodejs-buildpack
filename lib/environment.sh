...
write_export() {
  local bp_dir="$1"
  local build_dir="$2"
  echo "export PATH=\"$build_dir/.heroku/node/bin:\$PATH:$build_dir/node_modules/.bin\"" > $bp_dir/buildpack-export
  echo "export NODE_HOME=\"$build_dir/.heroku/node\"" >> $bp_dir/buildpack-export
}
